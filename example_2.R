...
symmetrize <- function(X) { return((X + t(X)) / 2) }

matrix_sqrt <- function(P) {
  eigP <- eigen(P)
  return(eigP$vectors %*% diag(sqrt(eigP$values)) %*% t(eigP$vectors))
}
...
dmatrixbeta_typeII <- function(X, A, B) {
  ...
  det_X      <- det(X)
  det_I_plus <- det(diag(d) + X)
  return(num / den * det_X^(A - (d + 1)/2) * det_I_plus^(-(A + B)))
}

pvalue_matrix_beta_MC <- function(T_obs, A, B, n_MC = 1e4) {
  f_obs <- dmatrixbeta_typeII(T_obs, A, B)
  SPD_list <- lapply(1:n_MC, function(x) {
    X <- matrixsampling::rmatrixbeta(1, d, A, B, def = 2, checkSymmetry = TRUE)[,,1]
    symmetrize(as.matrix(solve(diag(d) - X) %*% X))
  })
  indicators <- sapply(SPD_list, function(X) {
    fX <- dmatrixbeta_typeII(X, A, B)
    as.numeric(fX >= f_obs)
  })
...
T_A  <- conjugate(conjugate(SOP_A, Sigma_hat), Vinv)
T_B  <- conjugate(conjugate(SOP_B, Sigma_hat), Vinv)
T_AB <- conjugate(conjugate(SOP_AB, Sigma_hat), Vinv)
...
# ## ----- 6. Univariate two-way ANOVAs (from scratch, diamonds version) -------
# 
