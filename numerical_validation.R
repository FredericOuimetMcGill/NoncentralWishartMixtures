...
## Written by Frederic Ouimet (July 2025)
...
simulate_rep <- function(rep) {
  ...
  # Compute means and sums-of-cross-products (SOPs)
  ...
  SOP_A <- matrix(0, nrow = d, ncol = d)
  SOP_B <- matrix(0, nrow = d, ncol = d)
  SOP_AB <- matrix(0, nrow = d, ncol = d)
  SOP_E <- matrix(0, nrow = d, ncol = d)
  ...
  SOP_E_norm <- solve(conjugate(SOP_E, Sigmainv))
  
  T_A  <- conjugate(conjugate(SOP_A, Sigmainv), SOP_E_norm)
  T_B  <- conjugate(conjugate(SOP_B, Sigmainv), SOP_E_norm)
  T_AB <- conjugate(conjugate(SOP_AB, Sigmainv), SOP_E_norm)
